...
// wtf peoples static char *slider_get_laserdisc_desc(const device_config *screen);
#ifdef MAME_DEBUG
...
int ui_display_startup_screens(running_machine *machine, int first_time, int show_disclaimer)
{
	...
	for (state = 2; state < maxstate && !mame_is_scheduled_event_pending(machine) && !ui_menu_is_force_game_select(); state++)
	{
...
static char *slider_get_laserdisc_desc(const device_config *laserdisc)
{
	...
	return descbuf;
#else
	return "LASERDISC NOT SUPPORTED";
#endif	
}
